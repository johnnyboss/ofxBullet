...
void ofxBulletSoftTriMesh::create( ofxBulletWorldSoft* a_world, ofMesh& aMesh, btTransform &a_bt_tr, float a_mass ) {
    ...
    static const int totalIndices  = (int)aMesh.getNumIndices();
    bullet_vertices  = new btScalar[ totalVerts * 3 ];
   // int bullet_indices[ totalIndices ][3];//         = int[ totalIndices/3 ][3];
	vector < ofPoint> bullet_indices;
	bullet_indices.resize((int)(totalIndices/3));
    ...
    for( int i = 0; i < totalIndices; i += 3 ) {
        bullet_indices[i/3].x = tindices[i+0];
        bullet_indices[i/3].y = tindices[i+1];
        bullet_indices[i/3].z = tindices[i+2];
    }
	const int ntri = (int)bullet_indices[0].x;
